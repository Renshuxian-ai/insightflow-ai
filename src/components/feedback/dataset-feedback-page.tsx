  ...
  return (
    <main className="desktop-density-page mx-auto w-full px-5 py-7 sm:px-7 lg:px-6 lg:py-4">
      <header className="flex flex-col justify-between gap-5 border-b border-[#e6e9ef] pb-6 sm:flex-row sm:items-end lg:gap-2 lg:pb-3">
        ...
        <span className="w-fit rounded-md bg-[#edf1ff] px-2 py-1 text-[10px] font-bold tracking-[0.08em] text-[#3559e8] lg:px-2 lg:py-0.5 lg:text-[9px]">
          UPLOADED DATASET
        </span>
      </header>
      ...
      <section className="mt-6 lg:mt-4" aria-labelledby="feedback-overview-title">
        <div className="mb-3 flex items-center justify-between gap-4 lg:mb-2.5 lg:gap-3">
          ...
          <p className="text-xs text-[#98a1b1] lg:text-[11px]">Observed dataset evidence</p>
        </div>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            <article key={metric.label} className="rounded-xl border border-[#e7eaf0] bg-white p-5 lg:min-h-[88px] lg:p-3">
              <p className="text-[13px] font-medium text-[#657084] lg:text-[11px] lg:leading-4">{metric.label}</p>
              <p className="mt-3 text-[25px] font-semibold tracking-[-0.04em] text-[#172033] lg:mt-1.5 lg:text-xl lg:leading-6">{metric.value}</p>
              <p className="mt-2 text-xs leading-4 text-[#98a1b1] lg:mt-1.5 lg:text-[11px]">{metric.detail}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="mt-6 lg:mt-4" aria-labelledby="feedback-topics-title">
        <div className="mb-2.5 flex flex-wrap items-end justify-between gap-3 lg:mb-2.5">
          <div>
          ...
          <p className="text-xs text-[#68748a] lg:text-[11px]">
        ...
        <div className="grid gap-4 lg:grid-cols-2 lg:gap-3 2xl:grid-cols-3">
          {runtime.topics.map((topic) => (
            <button
              ...
              className={`min-h-[210px] cursor-pointer rounded-xl border bg-white p-5 text-left transition-colors lg:min-h-[132px] lg:p-3 ${
            ...
            >
              <div className="flex items-start justify-between gap-3 lg:gap-2">
                <div>
                  ...
                  <h3 className="mt-1.5 text-base font-semibold text-[#263247] lg:mt-0.5 lg:text-[13px] lg:leading-[18px]">{topic.title}</h3>
                </div>
                <span className={`rounded-md px-2 py-1 text-[10px] font-bold uppercase lg:px-1.5 lg:py-0.5 lg:text-[9px] ${sentimentStyles[topic.sentiment]}`}>
                  {topic.sentiment}
                </span>
              </div>
              <p className="mt-6 text-[25px] font-semibold tracking-[-0.04em] text-[#172033] lg:mt-1.5 lg:text-xl lg:leading-6">
                {topic.mentionCount}
              </p>
              <p className="text-xs text-[#8a94a6] lg:text-[10px]">mentions</p>
              <p className="mt-5 border-t border-[#eef0f4] pt-4 text-xs font-medium text-[#68748a] lg:mt-1.5 lg:pt-1.5 lg:text-[11px] lg:leading-4">
                {formatTrend(topic)}
              </p>
            </button>
          ))}
      ...
      {selectedTopic ? (
        <section className="mt-6 lg:mt-4" aria-labelledby="dataset-feedback-detail-title">
          <div className="mb-2.5 flex items-center justify-between gap-3 lg:mb-2.5">
            <div>
              <h2 id="dataset-feedback-detail-title" className="text-sm font-semibold text-[#344056] lg:text-[13px]">Topic detail</h2>
              <p className="mt-1 text-xs text-[#98a1b1] lg:text-[11px]">Observed evidence for {selectedTopic.title}</p>
            </div>
            {href ? (
              <InvestigationLaunchLink href={href} className="inline-flex h-9 items-center rounded-lg bg-[#3559e8] px-3.5 text-xs font-semibold text-white hover:bg-[#2446cb] lg:h-7 lg:px-2.5 lg:text-[11px]">
                Investigate <span aria-hidden="true" className="ml-1">→</span>
              </InvestigationLaunchLink>
            ) : (
              <span className="rounded-md bg-[#f1f3f6] px-2 py-1 text-[10px] font-bold text-[#687387] lg:px-2 lg:py-0.5 lg:text-[9px]">
                INVESTIGATION EVIDENCE INSUFFICIENT
              </span>
            )}
          </div>
          <div className="rounded-xl border border-[#e7eaf0] bg-white p-5 lg:p-3">
            <div className="flex items-center gap-2">
              <span className="rounded-md bg-[#edf1ff] px-2 py-1 text-[10px] font-bold uppercase text-[#3559e8]">Evidence</span>
              <h3 className="text-sm font-semibold text-[#344056] lg:text-[13px]">Representative feedback quotes</h3>
            </div>
            {selectedTopic.quotes.length > 0 ? (
              <div className="mt-4 grid gap-3 md:grid-cols-3 lg:mt-3">
                {selectedTopic.quotes.map((quote, index) => (
                  <figure key={`${selectedTopic.id}-${index}`} className="rounded-lg border border-[#e9ecf1] bg-[#fafbfc] p-4 lg:p-3">
                    <blockquote className="text-[13px] leading-5 text-[#526078] lg:text-xs lg:leading-[17px]">“{quote}”</blockquote>
                    <figcaption className="mt-3 text-[11px] text-[#98a1b1] lg:mt-2 lg:text-[10px]">Uploaded dataset</figcaption>
                  </figure>
                ))}
              </div>
            ) : (
              <p className="mt-3 text-xs text-[#98a1b1] lg:text-[11px]">Representative quote evidence is unavailable.</p>
            )}
          </div>
        </section>
      ) : null}
